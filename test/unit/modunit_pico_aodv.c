...
START_TEST(tc_get_node_by_addr)
{
    ...
    pico_tree_insert(&S->aodv_nodes, &a);

    fail_if(get_node_by_addr(S, &test) != &a);
    pico_tree_delete(&S->aodv_nodes, &a);
    fail_if(get_node_by_addr(S, &test) != NULL);

}
END_TEST

static int set_bcast_link_called = 0;
void pico_ipv4_route_set_bcast_link(struct pico_stack *S, struct pico_ipv4_link *link)
{
    IGNORE_PARAMETER(S);
...
int pico_ipv4_route_add(struct pico_stack *S, struct pico_ip4 address, struct pico_ip4 netmask, struct pico_ip4 gateway, int metric, struct pico_ipv4_link *link)
{
    IGNORE_PARAMETER(S);
    IGNORE_PARAMETER(link);
...
START_TEST(tc_aodv_elect_route)
{
    ...
    called_route_add = 0;
    aodv_elect_route(S, &node, NULL, 150, NULL);
    ...
    aodv_elect_route(S, &node, &gateway, 150, NULL);
    ...
    route_add_gw = 0u;
    node.metric = 22;
    aodv_elect_route(S, &node, &gateway, 15, NULL);
...
START_TEST(tc_aodv_peer_new)
{
    ...
    pico_stack_init(&S);
    new = aodv_peer_new(S, &addr);
    fail_if(!new);
    fail_if(!get_node_by_addr(S, &addr));
...
START_TEST(tc_aodv_peer_eval)
{
    ...
    node = aodv_peer_eval(S, &addr, 0, 0);
    fail_if(!node);
    fail_if((node->flags & PICO_AODV_NODE_SYNC) != 0); /* Not synced! */

    /* Case 1: retrieve, unsynced */
    node->metric = 42;
    node = aodv_peer_eval(S, &addr, 0, 0); /* Should get existing node! */
    fail_if(!node);
    ...
    pico_set_mm_failure(1);
    node = aodv_peer_eval(S, &addr, long_be(10), 1);
    fail_if(node);

    /* Case 3: existing node, setting the new sequence */
    addr.ip4.addr = 0x11224433;
    node = aodv_peer_eval(S, &addr, long_be(10), 1); /* Should get existing node! */
    fail_if(node->metric != 42);
...
struct pico_ipv4_link *pico_ipv4_link_by_dev(struct pico_stack *S, struct pico_device *dev)
{
    IGNORE_PARAMETER(S);
...
struct pico_device *pico_ipv4_link_find(struct pico_stack *S, struct pico_ip4 *ip4)
{
    IGNORE_PARAMETER(S);
...
START_TEST(tc_aodv_recv_valid_rreq)
{
    ...
    aodv_recv_valid_rreq(S, &node, &req, &info);
    fail_if(pico_socket_sendto_called > 0);
...
START_TEST(tc_aodv_parse_rack)
{
    ...
    aodv_parse_rack(S, NULL, NULL, 0, NULL);
}
END_TEST
