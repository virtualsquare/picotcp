...
START_TEST(pico_ipv6_pkt_too_big)
{
    ...
    fail_if(pico_ipv6_pmtu_get(S, &path_id) != pkt1_mtu);
}
END_TEST
...
START_TEST(pico_ipv6_pkt_too_big_ignorecode)
{
    ...
    struct pico_stack *S = NULL;
    struct mock_device *dummy_device;

    pico_stack_init(&S);

    dummy_device= pico_mock_create(S, NULL);
    ...
    pico_icmp6_process_in(S, NULL, f_echo_request);
...
START_TEST(pico_ipv6_path)
{
    ...
    for (i = 0; i < 0xff; i++) {
        ...
        fail_if(pico_ipv6_path_update(S, &path_id, default_mtu) != PICO_PMTU_ERROR);
    }
    /* Adding paths should be OK */
    for (i = 0; i < 0xff; i++) {
        path_id.dst.addr[10] = i;
        fail_if(pico_ipv6_path_add(S, &path_id, default_mtu + i) != PICO_PMTU_OK);
    }
    /* Retrieved PMTU should be the same */
    for (i = 0; i < 0xff; i++) {
        path_id.dst.addr[10] = i;
        fail_if(pico_ipv6_pmtu_get(S, &path_id) != default_mtu + i);
    }
    /* Adding existing paths should be OK */
    for (i = 0; i < 0xff; i++) {
        path_id.dst.addr[10] = i;
        fail_if(pico_ipv6_path_add(S, &path_id, default_mtu + i + 1) != PICO_PMTU_OK);
    }
    for (i = 0; i < 0xff; i++) {
        path_id.dst.addr[10] = i;
        fail_if(pico_ipv6_pmtu_get(S, &path_id) != default_mtu + i + 1);
    }
    /* Updating existing paths should be OK */
    for (i = 0; i < 0xff; i++) {
        path_id.dst.addr[10] = i;
        fail_if(pico_ipv6_path_add(S, &path_id, min_mtu + i) != PICO_PMTU_OK);
    }
    for (i = 0; i < 0xff; i++) {
        path_id.dst.addr[10] = i;
        fail_if(pico_ipv6_pmtu_get(S, &path_id) != min_mtu + i);
    }
    /* Updating existing paths to higher MTU value should not be OK */
    for (i = 0; i < 0xff; i++) {
        path_id.dst.addr[10] = i;
        fail_if(pico_ipv6_path_update(S, &path_id, min_mtu + i + 1) != PICO_PMTU_ERROR);
    }
    for (i = 0; i < 0xff; i++) {
        path_id.dst.addr[10] = i;
        fail_if(pico_ipv6_pmtu_get(S, &path_id) != min_mtu + i);
    ...
    for (i = 0; i < 0xff; i++) {
        path_id.dst.addr[10] = i;
        fail_if(pico_ipv6_path_del(S, &path_id) != PICO_PMTU_OK);
    }
    ...
    for (i = 0; i < 0xff; i++) {
        path_id.dst.addr[10] = i;
        fail_if(pico_ipv6_path_del(S, &path_id) != PICO_PMTU_ERROR);
    }
    fail_if(pico_ipv6_path_add(S, &path_id, min_mtu - 1) != PICO_PMTU_ERROR);
    fail_if(pico_ipv6_path_add(S, &path_id, 0) != PICO_PMTU_ERROR);
    fail_if(pico_ipv6_path_add(S, NULL, min_mtu) != PICO_PMTU_ERROR);
    fail_if(pico_ipv6_path_add(S, &path_id, min_mtu + 1) != PICO_PMTU_OK);
    fail_if(pico_ipv6_path_update(S, NULL, min_mtu) != PICO_PMTU_ERROR);
    fail_if(pico_ipv6_path_update(S, &path_id, 0) != PICO_PMTU_OK);
    fail_if(pico_ipv6_pmtu_get(S, &path_id) != min_mtu);
    fail_if(pico_ipv6_path_del(S, NULL) != PICO_PMTU_ERROR);
    fail_if(pico_ipv6_pmtu_get(S, NULL) != 0);
}
END_TEST
...
START_TEST(pico_ipv6_path_cache)
{
    ...
    fail_if(S->ipv6_path_cache_gc_timer.id == 0);
    ...
    fail_if(pico_ipv6_path_update(S, &path_id, default_mtu) != PICO_PMTU_OK);
    sleep(6);
    pico_stack_tick(S); /* Updated path available other paths are deleted */
    fail_if(pico_ipv6_pmtu_get(S, &path_id) != default_mtu);
    for (i = 0; i < 0xfe; i++) {
        path_id.dst.addr[10] = i;
        fail_if(pico_ipv6_pmtu_get(S, &path_id) != 0);
    }
    sleep(6);
