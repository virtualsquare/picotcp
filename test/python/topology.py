...
class Network:
  def __init__(self, topology, real=''):
    ...
    self.sock = "/tmp/topology/net"+str(self.n)
    self.nextn = 1
    ...
    if real != '':
      subprocess.call(["ifconfig",real,"172.16."+str(self.n)+".1", "netmask", "255.255.255.0", "up"])
      self.nextn = 2
...
class Host:
          ...
          if h.eth1 and h.eth2:
            ...
            if (h.eth2.net.n == net.n) and (self not in h.eth1.net.hosts):
              if h.eth1.net.n > net.n or h.nat == False:
                print("FOUND route to net " + str(h.eth1.net.n))
              ...
              if h.eth2.net.n > net.n or h.nat == False:
                print("FOUND route to net " + str(h.eth2.net.n))
            ...
            if routing:
              dst = "172.16."+str(dst_net)+".0"
              gw = "172.16."+str(gw_net)+"."+str(gw_n)
            ...
            if routing and gw_net > dst_net and h.nat == False:
              ...
              while dst_net > 0:
                ...
                self.routes.append(dst+":255.255.255.0:"+gw+":")
                dst_net -= 1
            elif routing and gw_net is not None and gw_net  < dst_net:
              dst_net += 1
  ...
  def parse_options(self, eth, delay, bw, loss):
    if delay != "" or  bw != "":
      mysock = eth.net.sock + "__" + str(eth.n)
      wirecmd = ['wirefilter', '-v', mysock + ":" + eth.net.sock]
      if delay != '':
        wirecmd.append("-d")
  ...
  def __init__(self, topology, net1=None, net2=None, gw=None, args="tcpecho:5555", delay1="", bw1="", delay2="", bw2="", loss1="", loss2=""):
    ...
    if net1:
      ...
      if args.startswith("dhcpclient"):
        ...
        self.cmd.append("--vde")
        vdeline = "eth1:"+mysock+':'+"172.16."+str(self.eth1.net.n)+"."+str(self.eth1.n)+":255.255.255.0:"
      if self.gw and re.search(r"172\.16\." + str(self.eth1.net.n), self.gw):
        vdeline +=self.gw+":"
    ...
    if net2:
      ...
      if args.startswith("dhcpclient"):
        ...
        self.cmd.append("--vde")
        vdeline = "eth2:"+mysock+':'+"172.16."+str(self.eth2.net.n)+"."+str(self.eth2.n)+":255.255.255.0:"
      if self.gw and re.search(r"172\.16\." + str(self.eth2.net.n) + ".", self.gw):
        vdeline +=self.gw+":"
      self.cmd.append(vdeline)
    self.args = args
    self.pop = None
