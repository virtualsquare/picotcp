...
/*** START NATBOX ***/
void app_natbox(struct pico_stack *stack, char *arg)
{
    char *dest = NULL;
    ...
    link = pico_ipv4_link_get(stack, &ipdst);
    if (!link) {
    ...
    pico_ipv4_port_forward(stack, pub_addr, short_be(5555), priv_addr, short_be(6667), PICO_PROTO_UDP, PICO_NAT_PORT_FORWARD_ADD);
