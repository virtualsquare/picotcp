...
\item \texttt{remote$\_$port} - Port number of the receiving socket
\end{itemize}
...
In the same way, \texttt{remote$\_$port} will contain the port number of the sending socket, unless a NULL is passed
from the caller.
...
\item \texttt{info} - Extended information about the packet containing this datagram. Only the fields "ttl" and "tos" are taken into consideration, while "dev" is ignored.
...
\item \texttt{port} - Local port number to bind with the socket
\end{itemize}
...
\item \texttt{port} - Local port number associated to the socket
...
\item \texttt{local$\_$port} - Port number of the local socket (pointer)
\end{itemize}
...
IP-address and \texttt{remote$\_$port} will contain the port number of the requesting socket.
