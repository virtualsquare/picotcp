...
#define dns_name_foreach_label_safe(label, name, next, maxlen)                 \
  for ((label) = (name),                                                       \
      (next) = (char *)((name) + *(unsigned char *)(name) + 1);                \
       (*(label) != '\0') && ((uint16_t)((label) - (name)) < (maxlen)) &&      \
       (((*label) & 0xC0) != 0xC0) && (*label <= 63);                          \
       (label) = (next),                                                       \
      (next) = (char *)((next) + *(unsigned char *)(next) + 1))
 ...
 *			ession pointer into account but not the second byte,
 *which acts like a trailing zero-byte
 * ****************************************************************************/
...
char *pico_dns_decompress_name(char *name, pico_dns_packet *packet) {
  ...
  if (*label & 0xC0) {
    ...
    dns_name_foreach_label_safe(label, label, next,
                                PICO_DNS_NAMEBUF_SIZE - decompressed_index) {
      uint8_t label_size = (uint8_t)(*label + 1);
      if (decompressed_index + label_size >= PICO_DNS_NAMEBUF_SIZE) {
        return NULL;
      }
 ...
 * ****************************************************************************/
static uint16_t pico_dns_url_get_reverse_len(const char *url, uint16_t *arpalen,
                                             uint16_t proto) {
 ...
 * ****************************************************************************/
static char *pico_dns_url_to_reverse_qname(const char *url, uint8_t proto) {
  ...
  /* If reverse IPv6 address resolving, convert to IPv6 arpa-format */
    ...
    memcpy(reverse_qname + 1u + STRLEN_PTR_IP6, PICO_ARPA_IPV6_SUFFIX, arpalen);
  }
#endif
...
char *pico_dns_qname_to_url(const char *qname) {
  char *url = NULL;
  char temp[PICO_DNS_NAMEBUF_SIZE] = {0};
...
int pico_dns_name_to_dns_notation(char *url, uint16_t maxlen) {
  ...
  while ((c = *++i) != '\0') {
    ...
    if ((uint16_t)(i - url) > (uint16_t)maxlen)
      break;
  }
  *lbl = (char)(i - lbl - 1);
...
int pico_dns_notation_to_name(char *ptr, uint16_t maxlen) {
  ...
  dns_name_foreach_label_safe(label, ptr, next, maxlen) { *label = '.'; }
 ...
 *  @return 0 on success, something else on failure.
 * ****************************************************************************/
int pico_dns_mirror_addr(char *ip) {
  uint32_t addr = 0;
...
void pico_dns_ipv6_set_ptr(const char *ip, char *dst) {
  ...
  for (i = 15; i >= 0; i--) {
    if ((j + 3) > 64)
      return; /* Don't want j to go out of bounds */
 ...
 *				   format. When reverse resolving, only the IP,
 *either IPV4 or IPV6, should be given in string format. f.e. => for IPv4:
 *"192.168.2.1"
 *						=> for IPv6:
 ...
 *				   generated depending on the protocol. Can be
 ...
 * ****************************************************************************/
static uint16_t pico_dns_question_fill_name(char **qname, const char *url,
                                            uint16_t qtype, uint8_t proto,
                                            uint8_t reverse) {
  uint16_t slen = 0;
...
pico_dns_question_create(const char *url, uint16_t *len, uint8_t proto,
  ...
  /* Fill name field */
  slen = pico_dns_question_fill_name(&(question->qname), url, qtype, proto,
                                     reverse);
 ...
 *					   to. When function returns, this will
 *point to location right after the flat copied DNS Resource Record.
 *  @return Returns 0 on success, something else on failure.
 * ****************************************************************************/
static int pico_dns_record_copy_flat(struct pico_dns_record *record,
                                     uint8_t **destination) {
  ...
  dest_rsuffix =
      (struct pico_dns_record_suffix *)(dest_rname + record->rname_length);
  dest_rdata =
      ((uint8_t *)dest_rsuffix + sizeof(struct pico_dns_record_suffix));
  ...
  *destination = (uint8_t *)(dest_rdata + short_be(record->rsuffix->rdlength));
  return 0;
}
 ...
 * ****************************************************************************/
static uint16_t pico_dns_record_size(void *record) {
  uint16_t size = 0;
 ...
 *  @return Returns 0 on success, something else on failure.
 * ****************************************************************************/
static int pico_dns_record_fill_suffix(struct pico_dns_record_suffix **suf,
                                       uint16_t rtype, uint16_t rclass,
                                       uint32_t rttl, uint16_t rdlength) {
 ...
 *				   data should contain a DNS name, the name in
 *the databuffer needs to be in URL-format.
 *  @param datalen The exact length in bytes of the _rdata-buffer. If data of
 *				   record should contain a DNS name, datalen
 *needs to be pico_dns_strlen(_rdata).
 *  @param rtype   DNS type of the resource record to be
 *  @return Returns 0 on failure, length of filled in rdata-member on success.
 *			Can differ from datalen-param because of URL to DNS Name
 *conversion.
 * ****************************************************************************/
static uint16_t pico_dns_record_fill_rdata(uint8_t **rdata, void *_rdata,
                                           uint16_t datalen, uint16_t rtype) {
  uint16_t _datalen = 0;
...
struct pico_dns_record *pico_dns_record_create(struct pico_stack *S,
  ...
  datalen =
      pico_dns_record_fill_rdata(&(record->rdata), _rdata, datalen, rtype);
...
int pico_dns_question_cmp(void *qa, void *qb) {
  ...
  /* Then compare qnames */
  return pico_dns_rdata_cmp(
      (uint8_t *)a->qname, (uint8_t *)b->qname, pico_dns_strlen(a->qname),
...
int pico_dns_record_cmp_name_type(void *ra, void *rb) {
  ...
  /* Then compare names */
  return pico_dns_rdata_cmp(
      (uint8_t *)(a->rname), (uint8_t *)(b->rname), (uint16_t)strlen(a->rname),
...
int pico_dns_record_cmp(void *ra, void *rb) {
  ...
  /* Then compare rdata */
  return pico_dns_rdata_cmp(a->rdata, b->rdata, short_be(a->rsuffix->rdlength),
                            short_be(b->rsuffix->rdlength),
                            PICO_DNS_CASE_SENSITIVE);
}
 ...
 *					 Make sure you clear out (set to 0) this
 *param before you call this function because it doesn't happen inside and each
 *size will be added to the initial value.
 *  @param node_size Helper-function for type-specific size-determination
 *  @return Amount of items in the tree.
 * ****************************************************************************/
static uint16_t pico_tree_size(struct pico_tree *tree, uint16_t *size,
 ...
 *				 Will point to location after current section on
 *return.
 *  @return 0 on success, something else on failure.
 * ****************************************************************************/
static int pico_dns_fill_packet_rr_section(struct pico_tree *rtree,
                                           uint8_t **dest) {
  struct pico_tree_node *node = NULL;
  struct pico_dns_record *record = NULL;
 ...
 *  @return 0 on success, something else on failure.
 * ****************************************************************************/
static int pico_dns_fill_packet_question_section(pico_dns_packet *packet,
                                                 struct pico_tree *qtree) {
  struct pico_tree_node *node = NULL;
  struct pico_dns_question *question = NULL;
  ...
  pico_tree_foreach(node, qtree) {
    ...
    if (question) {
      ...
      /* Copy the suffix */
      dest_qsuffix =
          (struct pico_dns_question_suffix *)(dest_qname +
 ...
 * ****************************************************************************/
static uint8_t *pico_dns_packet_compress_find_ptr(uint8_t *name, uint8_t *data,
                                                  uint16_t len) {
 ...
 *  packet-buffer.
 * ****************************************************************************/
static int pico_dns_packet_compress_name(uint8_t *name, uint8_t *packet,
                                         uint16_t *len) {
  ...
  while (*lbl_iterator != '\0') {
    ...
    compression_ptr =
        pico_dns_packet_compress_find_ptr(lbl_iterator, packet + 12, *len);
 ...
 * ****************************************************************************/
static int pico_dns_compress_record_sections(uint16_t qdcount, uint16_t count,
  ...
  for (i = 0; i < count; i++) {
    ...
    /* To get rdlength */
    rsuffix = (struct pico_dns_record_suffix *)(_iterator +
                                                pico_dns_namelen_comp(
                                                    (char *)_iterator) +
                                                1u);

    /* Move to next res record */
    _iterator = ((uint8_t *)rsuffix + sizeof(struct pico_dns_record_suffix) +
 ...
 * ****************************************************************************/
static int pico_dns_packet_compress(pico_dns_packet *packet, uint16_t *len) {
  ...
  for (i = 0; i < qdcount; i++) {
    ...
    /* Move to next question */
    iterator = (uint8_t *)(iterator + pico_dns_namelen_comp((char *)iterator) +
 ...
 * ****************************************************************************/
static uint16_t pico_dns_packet_len(struct pico_tree *qtree,
                                    struct pico_tree *antree,
                                    struct pico_tree *nstree,
                                    struct pico_tree *artree, uint8_t *qdcount,
                                    uint8_t *ancount, uint8_t *nscount,
                                    uint8_t *arcount) {
 ...
 * ****************************************************************************/
static pico_dns_packet *pico_dns_packet_create(struct pico_tree *qtree,
                                               struct pico_tree *antree,
                                               struct pico_tree *nstree,
                                               struct pico_tree *artree,
                                               uint16_t *len) {
  ...
  *len = pico_dns_packet_len(&_qtree, &_antree, &_nstree, &_artree, &qdcount,
  ...
  if (pico_dns_fill_packet_rr_sections(packet, &_qtree, &_antree, &_nstree,
                                       &_artree)) {
