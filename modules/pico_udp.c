...
uint16_t pico_udp_recv(struct pico_socket *s, void *buf, uint16_t len, void *src, uint16_t *port, struct pico_msginfo *msginfo)
{
    ...
    if (f) {
        if (!f->payload_len) {
            ...
            /* A datagram whose transport length is smaller than the UDP header
             * carries no payload; without this guard the subtraction underflows
             * to ~65528 and the copy below reads far past the frame
             * (FSCT-2026-0019). */
            if (f->transport_len < sizeof(struct pico_udp_hdr)) {
                f->payload_len = 0;
            } else {
